...
public class RetryTest {
	...
	List<Maybe<String>> testUuidList =
			Stream.concat(Stream.concat(Stream.generate(()->createMaybe(namedUuid("before1"))).limit(3),
					Stream.generate(()->createMaybe(namedUuid("before2"))).limit(3)),
					Stream.concat(Stream.generate(()->createMaybe(namedUuid("test"))).limit(3),
							Stream.generate(()->createMaybe(namedUuid("after"))).limit(3))).collect(Collectors.toList());
	...
	public void verify_retry_test_with_two_before_methods() {
		...
		ArgumentCaptor<StartTestItemRQ> startItems = ArgumentCaptor.forClass(StartTestItemRQ.class);
		// 2 before methods, 1 test, 1 after method, 3 retries for all of them = 12 start methods
		verify(launch, times(12)).startTestItem(same(testClassUuid), startItems.capture());
